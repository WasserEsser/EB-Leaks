    ...
    internal class ProjectAddonHandler : AddonHandler
    {
        internal override void Compile(ElobuddyAddon addon)
        {
            ...
            else
            {
                ...
                var outputPath = addon.GetOutputFilePath();
                var pdpPath = Path.Combine(Path.GetDirectoryName(outputPath), compileResult.PdbFileName ?? "");

                FileHelper.SafeWriteAllBytes(outputPath, compileResult.OutputFile);
                ...
                addon.Version = FileVersionInfo.GetVersionInfo(outputPath).FileVersion;
                addon.Hash = Md5Hash.ComputeFromFile(outputPath);
